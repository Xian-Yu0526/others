...
if __name__ == '__main__':
    ...
    dataset = Data_load(r"G:\anime_faces_img")
    data = Data.DataLoader(dataset=dataset, batch_size=100, shuffle=True,num_workers=4)

    d_net = D_net().cuda()
    g_net = G_net().cuda()
    if os.path.exists(r'./params2/p0.pt'):
        try:
            d_net.load_state_dict(torch.load(r'./params2/p0.pt'))
            g_net.load_state_dict(torch.load(r'./params2/p1.pt'))
            print("加载成功")
        except:
            print("加载不成功！")
    ...
    for epoch in range(num_epoch):
        for i,(img) in enumerate(data):
            ...
            z = torch.randn(img.size(0),128,1,1).cuda()
            ...
            if i%100== 0:
                ...
                torch.save(d_net.state_dict(),"./params2/p0.pt")
                torch.save(g_net.state_dict(), "./params2/p1.pt")
                print("保存成功")
                real_img = real_img.data.reshape(-1,3,96,96)
                fake_img = fake_img.data.reshape(-1,3,96,96)
                save_image(real_img,"./img2/{}-real_img.jpg".format(epoch+i),nrow=10,normalize=True,scale_each=True)
                save_image(fake_img,"./img2/{}-fake_img.jpg".format(epoch+i),nrow=10,normalize=True,scale_each=True)