...
if __name__ == '__main__':
    ...
    data = Data.DataLoader(dataset=dataset, batch_size=100, shuffle=True,num_workers=2)

    d_net = D_net().cuda()
    g_net = G_net().cuda()
    if os.path.exists(r'./params/p0.pt'):
        try:
            d_net.load_state_dict(torch.load(r'./params/p0.pt'))
            g_net.load_state_dict(torch.load(r'./params/p1.pt'))
            print("加载成功")
        except:
            print("加载不成功！")
    ...
    for epoch in range(num_epoch):
        for i,(img) in enumerate(data):
            ...
            if i%100== 0:
                ...
                torch.save(d_net.state_dict(),"./params/p0.pt")
                torch.save(g_net.state_dict(), "./params/p1.pt")
                print("保存成功")
